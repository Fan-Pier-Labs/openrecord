...
export default [
  ...
  // below silently stop seeing them.
  {
    ...
    rules: {
      ...
      // Round-4 zero-violation set (each canary-verified: a planted violation
      // fires before the zero is trusted).
      ...
      // Small measured set fixed in the enabling PR.
      "@typescript-eslint/prefer-includes": "error",
      "@typescript-eslint/no-unnecessary-template-expression": "error",
      "@typescript-eslint/prefer-find": "error",
      // `x as T` where `T` is just the non-null of x reads clearer as `x!`.
      "@typescript-eslint/non-nullable-type-assertion-style": "error",
      // Catch callbacks get `unknown`, matching useUnknownInCatchVariables.
