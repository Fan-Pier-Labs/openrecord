 ...
 *
 * Every field on those types is optional, so this checks the field *names* —
 * a typo, or a field no instance ever sent, does not compile — without
 * promising any value is there. `as LoadAllergies` would check the same names
 * and then claim the values too.
 */
