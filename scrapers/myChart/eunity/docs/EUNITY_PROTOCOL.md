...
- `src/main/scrapers/myChart/eunity/amf.ts` — AMF3 binary protocol: `AMF3Writer` class, `buildAmfCall()`, response parsers.
- `src/main/scrapers/myChart/eunity/session.ts` — `initializeAmfSession()`.
- `src/main/scrapers/myChart/eunity/download.ts` — `downloadImage()`, `downloadImagingStudyDirect()`.
