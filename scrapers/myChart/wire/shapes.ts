/**
 * What MyChart answers, endpoint by endpoint.
 *
 * Observed on three real instances by the capture harness behind
 * `fake-mychart/src/data/realShapes.ts` — so this is what we have seen, never a
 * contract Epic owes us. Three instances out of ~750, across two Epic releases,
 * with per-organization configuration on top.
 *
 * Two markers carry what the captures could not tell us:
 *
 * - `unknown` — the field was `null` on every instance captured. We saw no
 *   value, so we know no type. It is not `null`, which would read as settled.
 * - `unknown[]` — the array was empty everywhere, so we have never seen an
 *   element. It is not `never[]`, which would make reading one an error rather
 *   than an open question.
 *
 * A handful of containers were never captured populated at all; their shapes
 * come from the scraper's original hand-written response types (first commit,
 * cafcdff5, read off live responses in a browser) and are marked at the site.
 * Those are weaker evidence than a capture. See
 * https://github.com/Fan-Pier-Labs/openrecord/issues/484.
 *
 * Read a payload with `rec<LoadAllergies>(body)` from `../processors/read.ts`,
 * which checks the field names and leaves every value to `text()` / `num()` /
 * `list()`. Never `as` — that checks the same names and then lies about the
 * values, on an instance that omits one.
 *
 * When the captures are refreshed, update this file to match;
 * `__tests__/wireShapes.unit.test.ts` fails the build if the two disagree.
 */
