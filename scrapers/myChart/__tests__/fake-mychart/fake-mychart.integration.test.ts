...
import { fetchLabResultsRaw, listLabResults } from '../../chart/labs/labResults'
...
for (const mode of MOUNT_MODES) {
  describe(`fake-mychart integration (${mode} mount)`, () => {
    ...
    it('keeps the always-"Unknown" abnormal flag out of the standard shape, and derives nothing', async () => {
      // Captured Sep 2026 against two real instances: this field is the
      // literal "Unknown" on all 175 components, including the 13 outside
      // their own numeric range, and no other field in the payload holds a
      // verdict (#375). raw records it because MyChart sends it; standard
      // leaves it out rather than hand a client a flag-shaped value that
      // always says nothing.
      const raw = await fetchLabResultsRaw(session)
      expect(JSON.stringify(raw)).toContain('"abnormalFlagCategoryValue":"Unknown"')

      const lipid = (await listLabResults(session)).orders.find(o => o.orderName === 'Lipid Panel')!
      ...
      for (const c of components) {
        ...
        expect('abnormalFlag' in c.componentResultInfo).toBe(false)
        // Nothing derived took its place: the value and the range go through,
        // and the judgement is left to the client.
      ...
      // Every point of every trend gets the same treatment.
      for (const component of Object.values(lipid.historicalResults)) {
