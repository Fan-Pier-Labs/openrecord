 ...
 *
 * Those suites need a live server, so they sit outside the unit-test globs
 * (`__tests__/*.test.ts` does not reach the `fake-mychart/` subdirectory) and
 * are named one file at a time in package.json scripts. Adding a file to that
 * directory is therefore not enough to get it run — and a suite nobody runs
 * looks exactly like a suite that passes. `marge-and-reset.test.ts` sat unwired
 * for months for precisely this reason, going green in CI without executing.
 *
 ...
 * plain one. So this checks coverage across both rather than assuming that one
 * glob could cover the directory.
 */
