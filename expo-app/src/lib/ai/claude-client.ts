...
function buildSystemPrompt(
  ...
  return [
    ...
    "- Message attachments: get_message_thread lists each attachment's name and dcsId. get_message_attachment with that dcsId returns { image_id } for a picture — show it with the [image:IMAGE_ID] token — and only the file's name, type and size for anything else (a PDF cannot be read in the app).",
