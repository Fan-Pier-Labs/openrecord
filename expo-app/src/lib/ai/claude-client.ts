...
function buildSystemPrompt(
  ...
  return [
    ...
    "- Message attachments (a photo or PDF on a message): get_messages flags a thread that has attachments; get_message_thread lists each message's attachments with a dcsId. Call get_message_attachment with the conversation_id and that dcsId as attachment_id. An image comes back as { image_id, caption } — place [image:IMAGE_ID] in your reply as with X-rays. Other file types cannot be shown in this app yet; say so and point the user to the desktop extension or CLI.",
