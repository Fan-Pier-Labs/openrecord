...
#   version.json        the update manifest every shipped client polls,
...
# Regenerated rather than trusted: version.json is what already-installed
# clients read to find out they are behind, so a deploy that shipped a stale one
# would quietly tell everyone they were up to date. It is derived from each
# package's package.json, so this is a no-op unless a version was bumped.
echo "==> Regenerating version.json from the package versions"
...
upload_built "$SRC_DIR/version.json" "version.json" "public, max-age=300"
...
"${AWS[@]}" cloudfront create-invalidation \
  ...
  --paths "/index.html" "/demo.html" "/privacy.html" "/terms.html" "/" "/version.json" "/og-image.png" "/icon.svg" \
