...
- **`rendersMedia`** marks the one capability (`download_imaging_study`) whose payload isn't JSON:
  ...
  the payload; it sits after the dispatch, never in place of it.
...
- **CLI + npm package** (`npm-package/`) — `--action` accepts any capability id with repeated
  ...
  moment it is imported and the parity test has to import it. `runCapabilityAction` folds
  `--patient` into the args *after* coercion — the registry declares `patient`, not each capability,
  so coercing it would trip the unknown-argument check. The library exposes the same set as
