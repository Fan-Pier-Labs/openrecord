...
- **`returnsFile`** marks a capability (`get_message_attachment`, `download_billing_statement`)
  ...
  the Downloads folder (never overwriting, showing a small image inline, and putting a text file
  in the result when its own `return_content` flag asks and it fits — a PDF only ever gets the
  saved path, since no content block carries one to a model), the CLI writes it
