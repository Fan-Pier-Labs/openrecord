...
```bash
cd fake-mychart && bun run dev   # port 4000
```
...
flowchart TB
    ...
    subgraph lam["Lambdas"]
        AI["<code>openrecord-demo-ai</code><br/>chat turns for the demo AND<br/>the app's free tier"]
        SPEND[("DynamoDB<br/><code>openrecord-ai-spend</code>")]
        NEWS["<code>newsletter-signup</code><br/>logs to CloudWatch"]
        AI --> SPEND
    end
...
Deploys are per-service scripts; see the **Deployment** section of the root `CLAUDE.md`
for the exact commands and the read-only-AWS rule.
