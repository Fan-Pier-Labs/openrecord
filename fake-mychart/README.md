...
| **Care Team** | `careTeam.ts` | Dr. Julius Hibbert (PCP), Dr. Nick Riviera (Surgery), a third provider whose `Relation` is `null` (a role-less provider, as real instances send it), plus Dr. Marvin Monroe as an outside provider from `LoadExternal` |
