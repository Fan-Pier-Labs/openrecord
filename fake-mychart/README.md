...
curl -X POST http://localhost:4000/mode -H 'Content-Type: application/json' -d '{"responseDelaySeconds":110}'
curl http://localhost:4000/mode   # {"mode":"prefixed","discovery":"redirect","movedHost":null,"proxyDiscovery":"json","requireTerms":false,"epicVersion":"November 2025","failingEndpoints":[],"responseDelaySeconds":0}
```
...
- `responseDelaySeconds` — **how slow the instance is.** Every post-login request is held that many seconds before it is answered; login itself stays quick. For exercising a client's handling of a tool call that outruns its host's limit — the Claude Desktop extension's `check_pending_call` — by hand in Claude Desktop. Keep it under 120, the scrapers' per-request deadline, or every request fails instead; a chart read makes at least two gated requests, so `110` pushes any data tool past the extension's 3.5-minute deadline. `0` restores it. No re-login needed.
...
| **Care Team** | `careTeam.ts` | Dr. Julius Hibbert (PCP), Dr. Nick Riviera (Surgery), the health plan itself as a `Payer` entry and a provider whose `Relation` is `null` — both cases real instances send — plus Dr. Marvin Monroe as an outside provider from `LoadExternal`. `providerBios` answers `/api/Providers/GetProviderBioPrivate` for the three clinicians (NPI in plain digits, as the one captured instance serves it); any other id is that instance's 500 |
...
`src/lib/messageBody.ts` produces exactly that, and both the seeded threads and
anything sent or replied through the fake go through it — so a body posted as
text does not read back as the string that was posted. Undoing it is the
processor's job (`bodyText` in `conversations.processor.ts`), and a fixture that
skipped the wrapper would let a regression there pass the whole suite.
