 ...
 *
 * Epic never stores or returns a bare string here. A message a patient typed as
 * plain text comes back from `GetConversationList` as a `div.fmtConv` wrapper
 ...
 *
 * The fake serves the same thing, for both the seeded threads and anything sent
 * or replied through it, because the processor's job on a body is to undo
 * exactly this (`bodyText` in `conversations.processor.ts`). A fixture that
 * skipped the wrapper would let a converter regression pass the whole suite —
 * every body would already be the plain text the assertions expect.
 */
