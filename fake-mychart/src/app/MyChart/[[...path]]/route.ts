...
import {
  ...
  TEST_OTP_CODE, resolveActiveRecord, type FakeUser, type ConversationStore,
} from '@/lib/state';
 ...
 *
 * `Authentication/*` is the login flow itself. The rest is the *pre*-auth
 * surface: self-signup, activation-code signup and account recovery all exist
 * precisely for people who have no account to sign in with, so gating them on a
 * live session would make them unreachable — on the fake and on a real
 * instance alike. Each entry covers both the SPA shell a scraper GETs to pick
 * up its `__RequestVerificationToken` and the JSON endpoints it then POSTs.
 *
 * Being session-exempt does NOT mean being CSRF-exempt: real Epic still
 * requires `__RequestVerificationToken` on these `api/*` POSTs (confirmed
 * against a live instance — see claude-memory/mychart-signup-recovery-api.md),
 * and the signup/recovery scrapers fetch the shell first to obtain one. The
 * token check below therefore runs on them like anything else.
 */
function isOpenRoute(lower: string): boolean {
  return lower.startsWith('authentication/')
    || lower === 'signup'
    || lower.startsWith('signup/')
    || lower.startsWith('api/signup/')
    || lower === 'app/activation'
    || lower === 'app/account-recovery'
...
async function renderPost(request: NextRequest, { params }: { params: Promise<{ path?: string[] }> }) {
  ...
  if (!lower.startsWith('authentication/')) {
    ...
    //
    // This runs on the pre-auth signup/recovery endpoints too — they are
    // exempt from the session check below, never from the token check.
