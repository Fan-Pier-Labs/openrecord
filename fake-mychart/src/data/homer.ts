...
export const billingVisits = {
  ...
  Data: {
    ...
    // Epic pages GetVisits and lazy-loads the rows past the first page: the
    // activity renders them collapsed and hydrates one when it is expanded.
    // An unhydrated row is not empty, it is fabricated — `Description` is the
    // "{VisitType} at {Facility}" template with both slots blank, the
    // procedure and coverage lists are null, `HospitalAccountId` is an
    // encrypted handle rather than the HAR number, and every amount is the
    // string "$0.00" even when the visit was charged hundreds of dollars.
    // The service date is real; only the detail is missing.
    //
    // Copied from a live instance that answered with 25 such rows out of 45
    // alongside `ShowingAll: false`. Serving only hydrated rows would let a
    // processor that reads "$0.00" as a settled balance pass here and
    // undercount on a real chart, which is the bug this fixture exists to
    // catch, so the stubs are here on purpose and must not be "fixed" by
    // filling them in.
    UnifiedVisitList: [
      {
        GroupType: 0,
        ...
        StartDateAccessibleText: 'March 14, 2025',
      ...
      },
      {
        ...
        StartDateAccessibleText: 'August 2, 2024',
        Description: 'Visit at ',
        Patient: null,
        Provider: null,
        ProviderId: '',
...
/**
 * What `GetMoreVisits` returns for the two stubs in `billingVisits`: the same
 * rows, populated — real description, real amounts, `LevelOfDetailLoaded: 2`,
...
export const billingHydratedVisits = [
  {
    ...
    ProcedureList: [
      {
        BillingSystem: 2,
        Description: "Physical Therapy Evaluation - <span class='subtlecolor'>97161 (CPT®)</span>",
        Amount: '$250.00',
        PaymentList: null,
        InsuranceAmountDue: null,
        SelfAmountDue: '$90.00',
  ...
  },
  {
    ...
    ProcedureList: [
      {
        BillingSystem: 2,
        Description: "Office Visit, New Patient - <span class='subtlecolor'>99203 (CPT®)</span>",
        Amount: '$325.00',
        PaymentList: null,
        InsuranceAmountDue: null,
        SelfAmountDue: '$0.00',
