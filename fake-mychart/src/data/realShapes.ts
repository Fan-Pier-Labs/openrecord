...
// /insurance/coverages/getcoverages — the patient's own coverages. The
// legacy Insurance activity's coverage list: `GET /Insurance` is a shell whose
// whole body is an empty `<div id="coverages-list">`, and this is what fills
// it. Captured on four live instances (three November 2025, one August 2025);
// the coverage element's field set was identical on all four, and one of the
// four has no coverage on file, which is what makes five empty arrays an
// observed answer rather than a guess.
//
// Only ActiveCoverages was non-empty on any captured account. The other four
// buckets carry the same element shape here because the controller builds all
// five with the one `convertRawCoverages` call (`bundles/insurance-controllers`),
// so they are the same type by construction rather than by observation.
export const insuranceGetCoverages = {
  "ActiveCoverages": [
    {
      "CoverageId": "",
      "CoverageName": "",
