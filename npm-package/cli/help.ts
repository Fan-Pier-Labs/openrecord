...
export function renderCliHelp(options: CapabilityListOptions = {}): string {
  const lines: string[] = [
    ...
    '',
    '  Public information, no account or credentials needed:',
    '    --action search_mycharts --arg query=uchealth        Find a health system\'s MyChart hostname',
    '    --action lookup_npi --arg npi=1234567893             A provider, by National Provider Identifier',
    '    --action search_npi_registry --arg last_name=Hibbert --arg state=IL',
    '    --host <hostname> --action hospital-info',
    '                               The health system behind a MyChart hostname: support lines, the',
    '                               bookable provider directory with clinic addresses, billing entities.',
    '                               --arg specialties=a,b limits the crawl; --arg providers=false skips it',
    '',
