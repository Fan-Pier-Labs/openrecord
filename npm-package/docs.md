...
| `client.getConversationMessages(conversationId)` | `Promise<ConversationThread>` — the whole thread, paging past the five messages `listConversations` inlines. |
...
| `client.getInsurancePayers()` | `Promise<InsurancePayerCatalogue>` — `{ scope: 'organization', payers: InsurancePayer[] }`: the payers this organization offers when adding a coverage, identical for every patient on the instance |
