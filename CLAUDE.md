...
- **The one outbound path** (`scrapers/http.ts`): **Every request the scrapers send leaves through `scraperFetch`, and there is deliberately nowhere else to make one from.** It owns the three things every outbound request needs, none of which survives being reimplemented at a call site: the Chrome header block (MyChart and the eUnity image servers answer a browser, not a bare `fetch`), the cookie jar wiring (load-balancer and bot-check cookies get set mid-redirect-chain and are expected back on the next hop), and the per-host permit (below). `MyChartRequest.makeRequest` builds MyChart URLs and follows redirects on top of it; the eUnity imaging scraper calls it directly with its own jar; the directory script does too. A second raw-fetch path is how the cap silently stops applying — it keeps working, it just isn't limited — so `http.test.ts` greps `scrapers/` and fails the build if a network call appears outside `http.ts`. Add a request, don't add a path.
  ...
  - **Test seams**, in place of injection: `setTestTransport(fn)` in `http.ts` routes every request process-wide (used by `loginFlow.test.ts`, which drives `myChartUserPassLogin` against a scripted server — **clear it in `afterEach`**), and `req.transport = fn` overrides one session. Both sit *below* the headers, the jar and the permit, so a test still exercises the request production would send. `req.transport` is null in production; anything that wraps it must call `platformFetch`, not the old value — see `probeMountDiscovery.test.ts`, which exists because binding the old value broke the whole 750-host sweep and nothing else caught it.
...
- **CLO image parser** (`scrapers/myChart/clo-image-parser/`): eUnity CLO image format decoder and encoder. Tests are colocated here rather than in `__tests__/`, which is why they went unrun for so long — the root `test` script used to name each test directory explicitly. It now discovers every test file in the repo (see Key Commands), so this directory needs no special handling. **The codec is bit-exact**: `encodePixelFile` → `convertCloToBitmap` round-trips every pixel losslessly, odd subband dimensions included (510x510 halves to a 255x255 subband), so round-trip tests assert equality and not a tolerance. **Any tolerance belongs to the image codec on the far side, never to the wavelet.** `convertCloToJpg` only special-cases `.webp`, so every other output extension gets JPEG bytes whatever the file is named, and JPEG at quality 100 is still lossy — its DCT quantisation rounds curved and diagonal edges by ±1 on a few percent of pixels while leaving axis-aligned content exact. Measure the codec against `convertCloToBitmap`; reach for a tolerance only when a test deliberately goes through an image encoder.
...
- `bun run test` — `test:unit` then `test:integration`: the full local run. **Needs a fake-mychart on :4000** for the integration half but **no real credentials** — nothing in `test` ever touches a real MyChart. CI runs `test:unit` and `test:integration` as separate jobs because only one of them starts a server. Joined with `&&` rather than `;` on purpose: with `;` a unit failure would be masked by the integration exit code.
- `bun run test:unit` — Every unit test (scrapers, shared, CLI, expo-app libs, desktop extension). **Discovers every `*.test.ts` in the repo** rather than naming test directories, so a new test file anywhere runs in CI the moment it is added — no package.json edit, and no way to add a test that silently never runs. It previously listed each directory, which is how all three `clo-image-parser` test files sat unrun indefinitely. **The only exclusions are the three tiers below**, each structural rather than a named file, and each with its own CI job: `**/__tests__/fake-mychart/` (`test:integration`), `tests/integration/ci/` (`test:ci-integration`), and `**/__tests__/integration/` (`test:real-mychart`). **A test that needs a server or credentials belongs in one of those folders** — that is what keeps the exclusion list three structural patterns instead of a growing list of filenames, and an exclusion without a job to run it is how a test goes dark. **Needs `cd claude-desktop-extension && bun install` first** — the capability-parity test imports the extension's real `registerAllTools`, so it needs `zod` and the MCP SDK from that package. Without them you get `Cannot find package 'zod'` and five failing parity tests.
- `bun run test:integration` — Every suite that runs against a **fake-mychart on :4000** (`FAKE_MYCHART_HOST` to point elsewhere). One substring filter, `__tests__/fake-mychart`, collects all of them across every package — the scrapers, the Desktop extension and the npm package each keep their own such folder, so a new integration suite anywhere is picked up with no script edit. The npm suite imports the built artifact, so `cd npm-package && bun run build` first.
- `bun run test:coverage` — The unit suite **and** the fake-mychart suite in one process, with coverage, enforcing the 75% per-file minimum (see Code Coverage Gate). Needs a fake-mychart on :4000. Both halves together on purpose: measuring the unit suite alone counts every scraper that is only reached end-to-end as untested. **npm-package's fake-mychart suite is the one exclusion** — it imports the built `dist/` bundle rather than source, and a single bundled megafile has no business in a per-file gate.
- `bun run test:real-mychart` — **The only script that talks to a real MyChart**, and the only one needing real credentials. Kept apart from `test` on purpose so neither a local `bun run test` nor CI can reach a live patient account.
...
The `fake-mychart` CI job starts the server and then runs `bun run test:integration` once — that single command covers the scraper suite, the desktop-extension suite and the npm-package suite, because each lives in its own `__tests__/fake-mychart/` folder and the filter matches all of them. That suite includes `scrapers/myChart/__tests__/fake-mychart/credential-setup.test.ts`, which drives `setupTotp`/`disableTotp` and `setupPasskey`/`listPasskeys`/`deletePasskey` **directly** rather than through the CLI, so a scraper break reports as a scraper failure instead of a CLI-output assertion. It is the layer below `tests/integration/ci/cli-passkey.test.ts`, which covers the same ground through the built binary.
...
- **The keys must be plural** — `lines`, `functions`, `statements`. Bun's own docs show `line`/`function`; those spellings are parsed and then **silently ignored**, leaving the gate reading as configured while enforcing nothing. `bun test -c <file>` is ignored the same way, which is why the settings live in the repo-root `bunfig.toml`. They only activate under `--coverage`, so `bun run test`, `test:integration` and `test:ci-integration` are unaffected.
