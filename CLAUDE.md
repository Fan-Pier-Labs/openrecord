...
- **Log redaction** (`shared/redact.ts`): The scraper's debug stream is verbose by design — it is the only diagnostic available on an instance nobody here can reach — but the auth endpoints it narrates hand back real credentials: the TOTP shared secret from `/api/secondary-validation/TotpQrCode`, the hidden `__RequestVerificationToken` on every MyChart form, WebAuthn challenges, `Set-Cookie` session cookies, and the live 2FA codes themselves. **Nothing derived from a response body, a header map, or a parsed auth payload goes to `logger.*` raw** — it goes through `redactBody`, `redactHeaders`, `redactJson`, `redactSecret` or `redactUrl` first. None of the log destinations are ephemeral (the desktop extension writes to Claude Desktop's log files, the CLI to stdout, the mobile app to the device console), so a secret that reaches a log outlives the session. The key matcher is deliberately broad — over-redacting a debug line costs a little context, under-redacting one writes a credential to disk — and booleans survive redaction so flags like `IsPasswordValid` stay readable. `secretLogging.test.ts` drives the real login / TOTP / passkey / T&C flows with scripted responses carrying known secrets and asserts none of them reach the sink; it is what catches a newly added `logger.debug(body)`.
