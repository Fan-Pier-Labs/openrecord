...
- **CLI** (`npm-package/cli/cli.ts`): Headless CLI entry point — bundled into the published `mychart-connector` npm package as the `mychart-cli` bin. `npm i -g mychart-connector` puts `mychart-cli` on PATH. Great for Claude code to use for testing changes in the cli or scrapers.
