...
- **CLO image parser** (`scrapers/myChart/clo-image-parser/`): eUnity CLO image format decoder and encoder. **`convertCloToJpg` writes JPEG and only JPEG**, and throws if `outputPath` doesn't end in `.jpg`/`.jpeg` rather than writing one format under another name. It used to special-case `.webp` and send every other extension to the JPEG encoder, so `out.png` received JPEG bytes under a PNG name — fine in any viewer, since they sniff the magic rather than the name, until something trusts the extension. **Other formats are `exporters/`' job, not this function's**: decode with `convertCloToBitmap16` and call `convertBitmap16ToPng` / `ToWebp` / `ToAvif` / `ToTiff`. Teaching the wrapper every format would just be a second dispatch list to keep in step with that directory. Omit `outputPath` to get the JPEG buffer back.
