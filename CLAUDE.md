...
Accounts with proxy access to several patients' charts (a parent reading a child's record) get `list_proxy_targets` and `switch_proxy_target`. Switching changes which record every other tool reads from, and is verified against the profile page before it returns — a switch that lands on a different patient fails instead of returning the wrong chart. Record ids are opaque and organization-specific, so `switch_proxy_target` takes `self: true` to return to the account holder rather than requiring a looked-up id.
