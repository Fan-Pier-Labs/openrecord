...
- **CLO image parser** (`scrapers/myChart/clo-image-parser/`): eUnity CLO image format decoder and encoder. **`convertCloToJpg`'s output format comes from the output path's extension** — `.jpg`/`.jpeg`, `.png`, `.webp`, `.avif`, `.tif`/`.tiff` — and an unrecognised extension throws rather than picking one. It used to special-case `.webp` and send everything else to the JPEG encoder, so `out.png` received JPEG bytes under a PNG name; that opens fine everywhere (viewers sniff the magic, not the name) until something trusts the extension. JPEG and WebP go through the 8-bit `convertBitmapTo*` helpers, whose byte-for-byte output callers depend on; the formats that can carry more than 8 bits decode to 16-bit and stay there, so a PNG of a 16-bit study isn't quantised to 256 levels on the way out. Omit `outputPath` to get a JPEG buffer back, as before.
