...
- `bun run test` — Run all unit tests (scrapers, shared, CLI, expo-app libs, desktop extension). **Discovers every `*.test.ts` in the repo** rather than naming test directories, so a new test file anywhere runs in CI the moment it is added — no package.json edit, and no way to add a test that silently never runs. It previously listed each directory, which is how all three `clo-image-parser` test files sat unrun indefinitely. Only suites needing something the unit run does not provide are excluded, via `--path-ignore-patterns`, and **each one has its own CI job**: `__tests__/integration/` (real credentials, `test:integration`), `__tests__/fake-mychart/` (live server, `test:fake-mychart`), `npm-package/src/__tests__/` (built `dist/` + server), `imaging/__tests__/download-study.test.ts` (live server), and `tests/integration/ci/` (Docker, `test:ci-integration`). Adding an exclusion without a job to run it is how a test goes dark — don't. **Needs `cd claude-desktop-extension && bun install` first** — the capability-parity test imports the extension's real `registerAllTools`, so it needs `zod` and the MCP SDK from that package. Without them you get `Cannot find package 'zod'` and five failing parity tests.
