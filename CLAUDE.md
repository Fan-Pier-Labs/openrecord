...
- **Every outbound request leaves through `scraperFetch` (`scrapers/http.ts`)** — it owns the browser
  headers, the cookie jar, the per-host permit, and the 60s deadline. A second fetch path loses all
  four; `http.unit.test.ts` fails the build if one appears. **There is no injectable `fetchFn`** —
