...
Not blockers, but where to spend the next test-writing effort: `eunity/imagingDirectDownload.ts` and `eunity/imagingViewer.ts` (need a live eUnity server), `setupTotp.ts` / `setupPasskey.ts` (interactive flows), `login.ts`, and the scraper-tool handler bodies in `claude-desktop-extension/src/tools.ts` (each needs its scraper mocked; only the shared error path is covered today). In `clo-image-parser/`, `clo_to_bitmap.test.ts` and `exporters.test.ts` run; `generate_clo.unit.test.ts` does not, because nothing in the test globs matches `*.unit.test.ts` yet.
