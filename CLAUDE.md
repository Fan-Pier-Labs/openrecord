...
- `bun run test:integration` — Run every `*.integration.test.ts` (needs fake-mychart on :4000, the built CLI, and Docker Compose up)
- `bun run test:mychart` — Run every `*.mychart.test.ts` against a **real** MyChart account. Requires credentials, never runs in CI, only ever run by hand.
- `bun run test:fake-mychart` — Run just the scraper integration suite against a running fake-mychart on port 4000
...
- **The real-MyChart suite is out of CI by construction.** No workflow globs `.mychart`, so a new real-account test cannot be swept into a CI run by someone adding a directory to a list. This is also why the sub-package steps run `bun run test` rather than a bare `bun test` — a bare invocation picks up *every* test file in the package.
...
`scrapers/myChart/clo-image-parser/generate_clo.test.ts` is the single deliberate exception: two of its CLO encode→decode assertions fail (the encoder is off by one on curved and diagonal content) and it has never run in CI, so it carries no kind — which is what keeps it out of every suite, with no per-file exclusion to find or forget. Fixing the encoder and renaming it to `.unit.test.ts` is tracked separately; do not silence it by deleting the assertions.

**Bun runs test files in directory-entry order, not alphabetically.** That order changes whenever a file in the directory is added or renamed, so a suite that depends on a neighbour having run first breaks for reasons that look nothing like the cause. Everything sharing the fake-mychart server must therefore reset it in its own `beforeAll` — see `resetFakeMyChart` in `scrapers/myChart/__tests__/fake-mychart/mountMode.ts`. Resetting only on the way *out* is not enough: it still leaves the first suite of a run trusting the previous `bun test` invocation.
...
`bun run test:ci-integration` scopes a run to just this directory; `bun run test:integration` runs it alongside every other integration suite in the repo, which needs the same services up.
...
The one exclusion is `tests/integration/ci/`, dropped with `--path-ignore-patterns` rather than by narrowing the suffix globs: it needs Docker Compose and drives the CLI as a subprocess, so nothing there is instrumented anyway.

**It needs a fake-mychart on `:4000`, npm-package's `dist/` built, and the extension's deps installed**, so the CI step lives in the `fake-mychart` job, which already has all three — merging the suites costs no extra setup. Locally: `cd fake-mychart && bun run dev`, then `cd npm-package && bun install && bun run build`. Measuring the unit suite alone counts every scraper that is only covered end-to-end as untested.
...
Not blockers, but where to spend the next test-writing effort: `eunity/imagingDirectDownload.ts` and `eunity/imagingViewer.ts` (need a live eUnity server), `setupTotp.ts` / `setupPasskey.ts` (interactive flows), `login.ts`, and the scraper-tool handler bodies in `claude-desktop-extension/src/tools.ts` (each needs its scraper mocked; only the shared error path is covered today). `clo-image-parser/generate_clo.test.ts` is **not** wired into any suite — it is the one test file in the repo with no kind suffix, because two of its CLO encode→decode round-trip assertions fail on curved/diagonal content (off by one), and it has never run in CI; `clo_to_jpg.unit.test.ts` and `exporters.unit.test.ts` from that directory do run.
