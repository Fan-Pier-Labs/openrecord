...
- `bun run fake-mychart` — Run the fake MyChart dev server on a **random free port in 4000-5000**, printed in a banner at startup (`cd fake-mychart && bun run dev` is the same script). Random so several Claude instances / worktrees can each run their own; pin it with `PORT=4000` when something needs a fixed port. Port selection lives in `fake-mychart/scripts/dev-server.ts`.
