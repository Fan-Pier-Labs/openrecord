...
- **Never log a secret** (`scrapers/myChart/__tests__/secretLogging.test.ts`): The scraper's debug stream is verbose by design — it is the only diagnostic available on an instance nobody here can reach — but the auth endpoints it narrates hand back real credentials: the TOTP shared secret from `/api/secondary-validation/TotpQrCode`, the hidden `__RequestVerificationToken` on every MyChart form, WebAuthn challenges, `Set-Cookie` session cookies, and the live 2FA codes themselves. **Response bodies, header maps and parsed auth payloads never go to `logger.*`** — log the status, the field *names*, or a boolean derived from the body, never the body. None of the log destinations are ephemeral (the desktop extension writes to Claude Desktop's log files, the CLI to stdout, the mobile app to the device console), so a secret that reaches a log outlives the session. `secretLogging.test.ts` drives the real login / TOTP / passkey / T&C flows with scripted responses carrying known secrets and asserts none of them reach the sink; it is what catches a newly added `logger.debug(body)`.
